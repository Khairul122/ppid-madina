...
class KategoriBerkalaModel {
    ...
    public function getAllDokumenBerkala($limit = 10, $offset = 0) {
        try {
            $stmt = $this->db->prepare("
                ...
                WHERE d.id_kategori = 1 AND d.status != 'draft'
            ...
            error_log("Error in getAllDokumenBerkala: " . $e->getMessage());
            return [];
        }
    }

    // Get total count of Berkala documents EXCEPT drafts
    public function getTotalCount() {
        try {
            $stmt = $this->db->prepare("SELECT COUNT(*) as total FROM dokumen WHERE id_kategori = 1 AND status != 'draft'");
            $stmt->execute();
    ...
    // Search documents in Berkala category EXCEPT drafts
    public function searchDokumenBerkala($search, $limit = 10, $offset = 0) {
        try {
            $searchTerm = "%{$search}%";
            $stmt = $this->db->prepare("
                SELECT d.*, k.nama_kategori, dp.nama_jenis as nama_dokumen_pemda
            ...
            error_log("Error in searchDokumenBerkala: " . $e->getMessage());
            return [];
        }
    }

    // Get all documents with category Berkala (id_kategori = 1) - ALL statuses
    public function getAllDokumenBerkalaAllStatus($limit = 10, $offset = 0) {
        try {
            $stmt = $this->db->prepare("
                SELECT d.*, k.nama_kategori, dp.nama_jenis as nama_dokumen_pemda
                FROM dokumen d
            ...
            error_log("Error in getAllDokumenBerkalaAllStatus: " . $e->getMessage());
            return [];
        }
    }

    // Get total count of Berkala documents - ALL statuses
    public function getTotalCountAllStatus() {
        try {
            $stmt = $this->db->prepare("SELECT COUNT(*) as total FROM dokumen WHERE id_kategori = 1");
            $stmt->execute();
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
            return $result['total'];
        } catch (Exception $e) {
    ...
    // Search documents in Berkala category - ALL statuses
    public function searchDokumenBerkalaAllStatus($search, $limit = 10, $offset = 0) {
        try {
            $searchTerm = "%{$search}%";
            $stmt = $this->db->prepare("
                SELECT d.*, k.nama_kategori, dp.nama_jenis as nama_dokumen_pemda
            ...
            error_log("Error in searchDokumenBerkalaAllStatus: " . $e->getMessage());
            return [];
        }
    }
    ...
    public function getDokumenById($id) {
        try {
            $stmt = $this->db->prepare("
                ...
                LEFT JOIN kategori k ON d.id_kategori = k.id_kategori
                LEFT JOIN dokumen_pemda dp ON d.id_dokumen_pemda = dp.id_dokumen_pemda
                WHERE d.id_dokumen = ? AND d.id_kategori = 1
            ");
    ...
    // Create new document in Berkala category
        ...
        try {
            $stmt = $this->db->prepare("
                ...
                SET judul = ?, kandungan_informasi = ?, terbitkan_sebagai = ?,
                    id_dokumen_pemda = ?, tipe_file = ?, upload_file = ?, status = ?
                WHERE id_dokumen = ? AND id_kategori = 1
            ");
            $stmt->bindParam(1, $data['judul'], PDO::PARAM_STR);
            $stmt->bindParam(2, $data['kandungan_informasi'], PDO::PARAM_STR);
            $stmt->bindParam(3, $data['terbitkan_sebagai'], PDO::PARAM_STR);
    ...
    public function deleteDokumen($id) {
        try {
            $stmt = $this->db->prepare("DELETE FROM dokumen WHERE id_dokumen = ? AND id_kategori = 1");
            $stmt->bindParam(1, $id, PDO::PARAM_INT);
            return $stmt->execute();
        } catch (Exception $e) {
