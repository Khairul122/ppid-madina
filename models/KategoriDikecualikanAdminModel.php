...
class KategoriDikecualikanAdminModel {
    private $db;
    ...
    public function getAllDokumenDikecualikan($limit = 10, $offset = 0) {
        try {
            $stmt = $this->db->prepare("
                ...
                WHERE d.id_kategori = 4 AND d.status != 'draft'
            ...
            error_log("Error in getAllDokumenDikecualikan: " . $e->getMessage());
            return [];
        }
    }

    // Get total count of Dikecualikan documents EXCEPT drafts
    public function getTotalCount() {
        try {
            $stmt = $this->db->prepare("SELECT COUNT(*) as total FROM dokumen WHERE id_kategori = 4 AND status != 'draft'");
            $stmt->execute();
    ...
    // Search documents in Dikecualikan category EXCEPT drafts
    public function searchDokumenDikecualikan($search, $limit = 10, $offset = 0) {
        try {
            $searchTerm = "%{$search}%";
            $stmt = $this->db->prepare("
                SELECT d.*, k.nama_kategori, dp.nama_jenis as nama_dokumen_pemda
            ...
            error_log("Error in searchDokumenDikecualikan: " . $e->getMessage());
            return [];
        }
    }

    // Get all documents with category Dikecualikan (id_kategori = 4) - ALL statuses
    public function getAllDokumenDikecualikanAllStatus($limit = 10, $offset = 0) {
        try {
            $stmt = $this->db->prepare("
                SELECT d.*, k.nama_kategori, dp.nama_jenis as nama_dokumen_pemda
                FROM dokumen d
            ...
            error_log("Error in getAllDokumenDikecualikanAllStatus: " . $e->getMessage());
            return [];
        }
    }

    // Get total count of Dikecualikan documents - ALL statuses
    public function getTotalCountAllStatus() {
        try {
            $stmt = $this->db->prepare("SELECT COUNT(*) as total FROM dokumen WHERE id_kategori = 4");
            $stmt->execute();
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
            return $result['total'];
        } catch (Exception $e) {
    ...
    // Search documents in Dikecualikan category - ALL statuses
    public function searchDokumenDikecualikanAllStatus($search, $limit = 10, $offset = 0) {
        try {
            $searchTerm = "%{$search}%";
            $stmt = $this->db->prepare("
                SELECT d.*, k.nama_kategori, dp.nama_jenis as nama_dokumen_pemda
            ...
            error_log("Error in searchDokumenDikecualikanAllStatus: " . $e->getMessage());
            return [];
        }
    }
    ...
    public function getDokumenById($id) {
        try {
            $stmt = $this->db->prepare("
                ...
                LEFT JOIN kategori k ON d.id_kategori = k.id_kategori
                LEFT JOIN dokumen_pemda dp ON d.id_dokumen_pemda = dp.id_dokumen_pemda
                WHERE d.id_dokumen = ? AND d.id_kategori = 4
            ");
