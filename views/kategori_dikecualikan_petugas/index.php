...
<body class="with-welcome-text">
          ...
          <div class="row">
            <div class="col-sm-12">
              ...
              <div class="d-flex justify-content-between align-items-center mb-3">
                <div class="d-flex align-items-center">
                  <span style="font-size: 18px; font-weight: 500;">Dokumen Dikecualikan SKPD</span>
                </div>

                <div class="d-flex gap-2">
                  <!-- Search Form -->
                  <form method="GET" class="d-flex">
                    <input type="hidden" name="controller" value="kategoridikecualikanpetugas">
                  ...
                  <button class="btn btn-warning" onclick="window.location.href='index.php?controller=kategoridikecualikanpetugas&action=draft'">
                    <i class="mdi mdi-file-document-edit me-1"></i>Draft
                  </button>

                  <?php if (!empty($dokumen_list)): ?>
                    <button class="btn btn-info" onclick="window.location.href='index.php?controller=kategoridikecualikanpetugas&action=export'">
                      <i class="mdi mdi-file-excel me-1"></i>Ekspor Excel
                    </button>
                  <?php endif; ?>

                  <button class="btn btn-primary" onclick="window.location.href='index.php?controller=kategoridikecualikanpetugas&action=create'">
                          ...
                          <td style="padding: 15px; border: none; text-align: center;">
                            <div class="d-flex justify-content-center gap-1 flex-wrap">
                              <button class="btn btn-sm"
                                      onclick="window.location.href='index.php?controller=kategoridikecualikanpetugas&action=edit&id=<?php echo $dokumen['id_dokumen']; ?>'"
                ...
                <nav aria-label="Page navigation" class="mt-3">
                  <ul class="pagination justify-content-center">
                    ...
                    <?php if ($pagination['current_page'] > 1): ?>
                      <li class="page-item">
                        <a class="page-link" href="?controller=kategoridikecualikanpetugas&action=index&page=<?php echo $pagination['current_page'] - 1; ?>&limit=<?php echo $pagination['limit']; ?><?php echo !empty($search) ? '&search=' . urlencode($search) : ''; ?>">
                          Previous
                        </a>
                      </li>
                    <?php else: ?>
                    ...
                    ?>
                      <li class="page-item">
                        <a class="page-link" href="?controller=kategoridikecualikanpetugas&action=index&page=1&limit=<?php echo $pagination['limit']; ?><?php echo !empty($search) ? '&search=' . urlencode($search) : ''; ?>">1</a>
                      </li>
                      <?php if ($start_page > 2): ?>
                    ...
                    <?php for ($i = $start_page; $i <= $end_page; $i++): ?>
                      <li class="page-item <?php echo $i == $pagination['current_page'] ? 'active' : ''; ?>">
                        <a class="page-link" href="?controller=kategoridikecualikanpetugas&action=index&page=<?php echo $i; ?>&limit=<?php echo $pagination['limit']; ?><?php echo !empty($search) ? '&search=' . urlencode($search) : ''; ?>">
                          <?php echo $i; ?>
                        </a>
                      </li>
                    <?php endfor; ?>
                    ...
                    <?php if ($end_page < $pagination['total_pages']): ?>
                      ...
                      <?php endif; ?>
                      <li class="page-item">
                        <a class="page-link" href="?controller=kategoridikecualikanpetugas&action=index&page=<?php echo $pagination['total_pages']; ?>&limit=<?php echo $pagination['limit']; ?><?php echo !empty($search) ? '&search=' . urlencode($search) : ''; ?>">
              ...
              <?php else: ?>
                <div class="text-center py-5">
                  ...
                  <h5 class="text-muted mt-3">
                    <?php if (!empty($search)): ?>
                      Tidak ada dokumen dikecualikan yang ditemukan untuk pencarian "<?php echo htmlspecialchars($search); ?>"
                    <?php else: ?>
                      Belum ada data dokumen dikecualikan untuk SKPD Anda
                    <?php endif; ?>
                  </h5>
                  <button class="btn btn-primary mt-2" onclick="window.location.href='index.php?controller=kategoridikecualikanpetugas&action=create'">
  ...
  <script>
    ...
    function copyLink(id) {
      ...
      const link = window.location.origin + '/index.php?controller=kategoridikecualikanpetugas&action=view&id=' + id;
    ...
    document.getElementById('confirmDraftBtn').addEventListener('click', function() {
      if (currentDokumenId) {
        ...
        fetch('index.php?controller=kategoridikecualikanpetugas&action=updateToDraft', {
          method: 'POST',
          body: formData
        })
    ...
    document.getElementById('confirmDeleteBtn').addEventListener('click', function() {
      if (currentDokumenId) {
        fetch('index.php?controller=kategoridikecualikanpetugas&action=destroy&id=' + currentDokumenId, {
          method: 'GET'
        })
        .then(response => response.json())
        .then(data => {
