...
<body class="with-welcome-text">
            ...
            <div class="col-sm-12">
              ...
              <div class="d-flex justify-content-between align-items-center mb-3">
                <div class="d-flex align-items-center">
                  ...
                  <span style="font-size: 18px; font-weight: 500;">Operasional / SKPD</span>
                </div>

                <div class="d-flex gap-2">
                  <!-- Search Form -->
                  <form method="GET" class="d-flex">
                    <input type="hidden" name="controller" value="skpd">
                  ...
                  <?php if (!empty($skpd_list)): ?>
                    <button class="btn btn-info" onclick="window.location.href='index.php?controller=skpd&action=export'">
                      <i class="mdi mdi-file-excel me-1"></i>Ekspor Excel
                    </button>
                  <?php endif; ?>

                  <button class="btn btn-primary" onclick="window.location.href='index.php?controller=skpd&action=form'">
                    <i class="mdi mdi-plus me-1"></i>Tambah operasional
                  </button>
                </div>
              </div>
                      ...
                      <?php foreach ($skpd_list as $skpd): ?>
                        <tr style="border-bottom: 1px solid #f0f0f0;">
                          ...
                          <td style="padding: 15px; border: none; text-align: center;">
                            <?php echo !empty($skpd['email']) ? htmlspecialchars($skpd['email']) : '-'; ?>
                          </td>
                          <td style="padding: 15px; border: none; text-align: center;">
                            <?php echo !empty($skpd['kategori']) ? htmlspecialchars($skpd['kategori']) : '-'; ?>
                          </td>
                          <td style="padding: 15px; border: none; text-align: center;">
                            <div class="d-flex justify-content-center gap-1">
                              <button class="btn btn-sm"
                                      onclick="viewDetail(<?php echo $skpd['id_skpd']; ?>)"
                                      style="background: #17a2b8; color: white; border: none; padding: 6px 12px; border-radius: 4px;"
                                      title="Lihat Detail">
                                <i class="mdi mdi-eye"></i> Detail
                              </button>
                              <button class="btn btn-sm"
                                      onclick="window.location.href='index.php?controller=skpd&action=form&id=<?php echo $skpd['id_skpd']; ?>'"
                                      style="background: #ffc107; color: #212529; border: none; padding: 6px 12px; border-radius: 4px;"
                                      title="Edit Data">
                                <i class="mdi mdi-pencil"></i> Edit
                              </button>
                              <button class="btn btn-sm"
                                      onclick="confirmDelete(<?php echo $skpd['id_skpd']; ?>, '<?php echo addslashes($skpd['nama_skpd']); ?>')"
                ...
                <nav aria-label="Page navigation" class="mt-3">
                  <ul class="pagination justify-content-center">
                    ...
                    <?php if ($pagination['current_page'] > 1): ?>
                      <li class="page-item">
                        <a class="page-link" href="?controller=skpd&action=index&page=<?php echo $pagination['current_page'] - 1; ?>&limit=<?php echo $pagination['limit']; ?><?php echo !empty($search) ? '&search=' . urlencode($search) : ''; ?>">
                          Previous
                        </a>
                      </li>
                    <?php else: ?>
                    ...
                    ?>
                      <li class="page-item">
                        <a class="page-link" href="?controller=skpd&action=index&page=1&limit=<?php echo $pagination['limit']; ?><?php echo !empty($search) ? '&search=' . urlencode($search) : ''; ?>">1</a>
                      </li>
                      <?php if ($start_page > 2): ?>
                    ...
                    <?php for ($i = $start_page; $i <= $end_page; $i++): ?>
                      <li class="page-item <?php echo $i == $pagination['current_page'] ? 'active' : ''; ?>">
                        <a class="page-link" href="?controller=skpd&action=index&page=<?php echo $i; ?>&limit=<?php echo $pagination['limit']; ?><?php echo !empty($search) ? '&search=' . urlencode($search) : ''; ?>">
                          <?php echo $i; ?>
                        </a>
                      </li>
                    <?php endfor; ?>
                    ...
                    <?php if ($end_page < $pagination['total_pages']): ?>
                      ...
                      <?php endif; ?>
                      <li class="page-item">
                        <a class="page-link" href="?controller=skpd&action=index&page=<?php echo $pagination['total_pages']; ?>&limit=<?php echo $pagination['limit']; ?><?php echo !empty($search) ? '&search=' . urlencode($search) : ''; ?>">
              ...
              <?php else: ?>
                <div class="text-center py-5">
                  <i class="mdi mdi-office-building-outline" style="font-size: 64px; color: #ccc;"></i>
                  <h5 class="text-muted mt-3">
                    <?php if (!empty($search)): ?>
                      Tidak ada SKPD yang ditemukan untuk pencarian "<?php echo htmlspecialchars($search); ?>"
                    <?php else: ?>
                      Belum ada data SKPD
                    <?php endif; ?>
                  </h5>
                  <button class="btn btn-primary mt-2" onclick="window.location.href='index.php?controller=skpd&action=form'">
                    <i class="mdi mdi-plus me-1"></i>Tambah SKPD Pertama
                  </button>
                </div>
              <?php endif; ?>
            </div>
      ...
      <div class="modal-content">
        <div class="modal-header" style="background: #4A90E2; color: white;">
          <h5 class="modal-title" id="detailModalLabel">
            <i class="mdi mdi-eye me-2"></i>Detail SKPD
          </h5>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <p>Apakah Anda yakin ingin menghapus SKPD "<span id="deleteName"></span>"?</p>
        ...
        <div class="modal-footer">
          ...
          <form method="POST" action="index.php?controller=skpd&action=delete" style="display: inline;">
  ...
  <script>
    ...
    function viewDetail(id) {
      ...
      // Fetch data
      fetch(`index.php?controller=skpd&action=detail&id=${id}&ajax=1`)
                ...
                <div class="card border-0 bg-light">
                  <div class="card-body">
                    <h6 class="card-title text-primary mb-2">
                      <i class="mdi mdi-identifier me-1"></i>ID SKPD
                    </h6>
                    <p class="card-text fw-bold">${data.id_skpd}</p>
                  </div>
                </div>
              </div>
              <div class="col-md-6">
                <div class="card border-0 bg-light">
                  <div class="card-body">
                    <h6 class="card-title text-primary mb-2">
                      <i class="mdi mdi-office-building me-1"></i>Nama SKPD
                    </h6>
              ...
              <div class="col-md-12">
                <div class="card border-0 bg-light">
                  <div class="card-body">
                    ...
                    <p class="card-text">${data.alamat || '<em class="text-muted">Tidak ada data</em>'}</p>
                  </div>
                </div>
              </div>
              <div class="col-md-6">
                <div class="card border-0 bg-light">
                  <div class="card-body">
                    ...
                    <p class="card-text">${data.telepon || '<em class="text-muted">Tidak ada data</em>'}</p>
                  </div>
                </div>
              </div>
              <div class="col-md-6">
                <div class="card border-0 bg-light">
                  <div class="card-body">
                    ...
                    <p class="card-text">${data.email || '<em class="text-muted">Tidak ada data</em>'}</p>
                  </div>
                </div>
              </div>
              <div class="col-md-6">
                <div class="card border-0 bg-light">
                  <div class="card-body">
                    ...
                    <p class="card-text">${data.kategori || '<em class="text-muted">Tidak ada data</em>'}</p>
                  </div>
                </div>
              </div>
            </div>
    ...
    document.getElementById('editFromDetail').addEventListener('click', function() {
      if (currentSkpdId) {
        window.location.href = `index.php?controller=skpd&action=form&id=${currentSkpdId}`;
      }
    });
